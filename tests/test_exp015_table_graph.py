"""Exp 015 pure-logic unit tests: table->graph construction (bidirectional row
chains), prompt tokenization/labels, structural features, and the generation
position fix.

Run:  pytest tests/test_exp015_table_graph.py -v   (CPU-only, no datasets/HF)
"""
import sys, os
...
def test_ragged_row_padded_with_empty_cell():
    ...
    # both directions on the single row pair
    assert (3, 1) in g.edges() and (3, 2) in g.edges() and (2, 3) in g.edges()
...
def test_single_column_table_has_no_row_edges():
    g = build_table_graph(["only"], [["1"], ["2"]])
    # nodes: 0=column, 1,2 = cells; with a single column there are no left
    # OR right neighbors, so edges are exclusively cell->column.
...
def test_spd_sanity_on_hand_built_table():
    ...
    # Row chain is now SYMMETRIC: both directions are length 1.
    assert spd[3, 2] == 1            # right cell -> left cell
    assert spd[2, 3] == 1            # left cell -> right cell (bidir chain)
    assert spd[3, 0] == 2            # right cell -> left column via chain
    # cell -> right cell -> its column gives a length-2 path now.
    assert spd[2, 1] == 2
    # Cell -> column edges remain DIRECTED: column is still a sink.
    assert spd[0, 2] == UNREACHABLE
    assert spd[1, 2] == UNREACHABLE
