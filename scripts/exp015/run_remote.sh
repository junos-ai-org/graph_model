...
#
# Usage (on pod): bash scripts/exp015/run_remote.sh
# Smoke: MAX_EXAMPLES=40 EVAL_MAX=20 PUSH_HUB=0 bash scripts/exp015/run_remote.sh
# Pre-reqs on the pod env: GH_TOKEN, HF_TOKEN, WANDB_API_KEY, CODE_REF
# (branch/commit of junos-ai-org/graph_model carrying src/experiments/exp015).
set -euo pipefail
export WANDB_RUN_GROUP="${WANDB_RUN_GROUP:-exp_015}"
...
OUT=/workspace/exp015
CKPT="${OUT}/runs/gtlm-graph-bidir-rows/final"

LOG=/workspace/run_exp015.log
exec > >(tee -a "$LOG") 2>&1
echo "=== [exp015] $(date -u +%FT%TZ) start (epochs=${EPOCHS} max_examples=${MAX_EXAMPLES} eval_max=${EVAL_MAX}) ==="
...
: "${CODE_REF:?CODE_REF required (junos graph_model branch/commit with exp015)}"
...
echo "[exp015] graph_model at $(git rev-parse --short HEAD) (ref=${CODE_REF})"

# ---- 2. Deps (same core pins as exp013 + networkx for the graph build) ----
python -m pip install -q transformers==4.50.3 peft==0.18.1 datasets==4.8.4 accelerate==1.13.0 \
...
python -m src.experiments.exp015.run prep --out "${OUT}" "${PREP_ARGS[@]}"

# ---- 5. Train ----
echo "=== [$(date -u +%FT%TZ)] train gtlm-graph-bidir-rows ==="
python -m src.experiments.exp015.run train --out "${OUT}" --epochs "${EPOCHS}"

# ---- 6. Checkpoint to Hub FIRST (survival path against pod death) ----
[ -d "${CKPT}" ] || { echo "FATAL: expected checkpoint ${CKPT} missing"; exit 1; }
if [ "${PUSH_HUB}" = 1 ]; then
  push_dir "${OUT}/runs/gtlm-graph-bidir-rows" "exp_015/gtlm-graph-bidir-rows" \
    "exp015 gtlm-graph-bidir-rows adapter+bias (all checkpoints)"
fi

# ---- 7. Eval on TableBench DP test (records flushed per example) ----
echo "=== [$(date -u +%FT%TZ)] eval gtlm-graph-bidir-rows ==="
python -m src.experiments.exp015.run eval --out "${OUT}" \
    --checkpoint "${CKPT}" --eval-max "${EVAL_MAX}"

# ---- 8. Push eval artifacts ----
if [ "${PUSH_HUB}" = 1 ]; then
  push_dir "${OUT}/eval/gtlm-graph-bidir-rows" "exp_015/gtlm-graph-bidir-rows/eval" \
    "exp015 gtlm-graph-bidir-rows DP-test eval (records + results)"
fi

echo "=== [exp015] $(date -u +%FT%TZ) DONE ==="
touch /workspace/DONE_exp015
