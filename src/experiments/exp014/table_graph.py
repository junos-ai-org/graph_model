"""Exp 014 — table → graph construction (pure logic, no heavy deps).
...
as a graph and handing it to GTLM's generic biases (SPD + RRWP + Magnetic):
  ...
  * edges: cell -> its column node; cell -> the cell to its LEFT
    (each row is a left-pointing chain, every cell also links to its column)
