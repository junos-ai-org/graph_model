...
Variant `gtlm-graph-bidir-rows`: Llama-3.2-1B + LoRA(r=16, q/k/v/o) + GTLM's generic
...
Usage (on a GPU pod, from the graph_model repo root):
    python -m src.experiments.exp015.run prep  --out /workspace/exp015
    python -m src.experiments.exp015.run train --out /workspace/exp015
    python -m src.experiments.exp015.run eval  --out /workspace/exp015 \
        --checkpoint /workspace/exp015/runs/gtlm-graph-bidir-rows/final
...
def stage_train(args, tokenizer):
    ...
    run_dir = out / "runs" / "gtlm-graph-bidir-rows"
    targs = TrainingArguments(
        ...
        run_name=os.environ.get("WANDB_RUN_GROUP", "exp_015") + "/gtlm-graph-bidir-rows",
...
def stage_eval(args, tokenizer):
    ...
    ds = TableBenchDP(seed=SEED, max_cells=200)
    out_dir = Path(args.out) / "eval" / "gtlm-graph-bidir-rows"
    ...
    with rec_path.open("a") as rec_f:
        for i, ex in enumerate(ds.test(apply_max_cells=False)):
            ...
            try:
                ...
                print(f"[gtlm-graph-bidir-rows] OOM on id={ex['id']} — skipped", flush=True)
                continue
            ...
            rec_f.flush()
            if n % 25 == 0:
                print(f"[gtlm-graph-bidir-rows] {n} scored ({i + 1} seen) "
    ...
    results = {
        "variant": "gtlm-graph-bidir-rows", "checkpoint": args.checkpoint,
    ...
    print(f"[gtlm-graph-bidir-rows] n={n} accuracy={acc:.4f} skipped={len(skipped)} -> {out_dir}")
